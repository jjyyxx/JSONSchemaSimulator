...
const { TRAVIS_BRANCH, TRAVIS_PULL_REQUEST, TRAVIS_PULL_REQUEST_BRANCH } = process.env
...
if (TRAVIS_PULL_REQUEST && TRAVIS_BRANCH === 'master') {
  if (TRAVIS_PULL_REQUEST_BRANCH === 'develop') {
    ...
    const current = JSON.parse(execSync('git show develop:package.json').toString('utf8')).version
    if (previous === current) {
